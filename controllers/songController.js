...
const SongController = {
  ...
  /**
   * Recomendacion de canciones basada en la emocion que muestra un usuario
   * GET /songs/popular/:email
   */

  async getRecommendationByEmotion(req, res) {
    const session = getSession();
    ...
    try {
      ...
      MATCH (u:User {email: $email})-[f:feeling]->(s:Song)-[:evokes]->(e:Emotion)
      WITH u, e, SUM(f.strength) AS emotionScore
      ORDER BY emotionScore DESC
      LIMIT 3
      WITH u, COLLECT(e) AS topEmotions

      MATCH (rec:Song)-[:evokes]->(re:Emotion)
      WHERE re IN topEmotions AND NOT (u)-[:feeling]->(rec)

      RETURN rec.title AS title, re.name AS emotion
      ORDER BY rand()
      LIMIT 5
      `;
      ...
      if (result.records.length === 0) {
        return res.status(404).json({
          success: false,
          message: 'No se encontraron recomendaciones por emoción para este usuario o el usuario no existe'
        });
      }
      ...
      res.status(200).json({
        success: true,
        message: 'Recomendaciones por emoción encontradas',
        data: songs,
        count: songs.length
      });
    }
      ...
      console.error('Error al buscar recomendaciones por emoción:', error);
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor',
        error: error.message
